...
```bash
delta$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
delta$ sam local start-api
delta$ curl http://localhost:3000/
```
...
```bash
delta$ sam logs -n HelloWorldFunction --stack-name delta --tail
```
...
```bash
aws cloudformation delete-stack --stack-name delta
```
