...
```bash
writer$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
writer$ sam local start-api
writer$ curl http://localhost:3000/
```
...
```bash
writer$ sam logs -n HelloWorldFunction --stack-name writer --tail
```
...
```bash
aws cloudformation delete-stack --stack-name writer
```
